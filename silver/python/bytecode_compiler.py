...
def compile_instruction(variables: list, instruction: Instruction) -> bytearray:
    ...
    if instruction.operation.name in ["add_byte"]:  # variables-only
        ...
        raise Exception(f"Unimplemented instruction \
...
def compile_frame(_variables: list, frame: Frame, base_index: int):
    ...
    Silver.Encoding.Frame.StartVariablesVector(builder,
                                               len(compiled_variables))
...
def compile_variable(variable, data_index=None, published_name=None):
    ...
    if published_name is not None:
        Silver.Encoding.Variable.StartPublishedNameVector(builder,
                                                          len(published_name))
...
def compile_function(function: Function, base_index: int):
    ...
    for i in function.decorators:
        if i.typ == "entry_frame":
            raise Exception("Entry frame decorator is redundant")
        elif i.typ == "return":
            has_return = True
            if len(variables) != 0:
            ...
            arg_offset = 1 if not has_return else 0
            arg_index = int(i.arg_index) - arg_offset
            if len(variables) != arg_index:
                raise Exception(f"Bad argument indexing -\
                        should be {len(variables)} (possible -1),\
                        but is {i.arg_index} - {has_return} return")
            args.append((i.arg_name, i.arg_bytewidth))
            variables.append((i.arg_name, i.arg_bytewidth))
        else:
            raise Exception(f"Unknown or invalid-on-function\
                    decorator {i.typ}")

    frames = []
    instructions = bytearray()
    for i in function.frames:
        frame, instructions_compiled = compile_frame(
                variables,
                i,
                len(instructions) + base_index
        )
    ...
    if has_return:
        Silver.Encoding.Function.AddReturnByteWidth(builder,
                                                    int(return_bytewidth))
