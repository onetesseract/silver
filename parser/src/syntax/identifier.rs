...
use nom::{IResult, character::is_alphabetic, bytes::complete::{take_while1, take, take_while}, combinator::fail};
...
impl<'a> Identifier<'a> {
    pub fn parse(input: &'a str) -> IResult<&'a str, Self> {
        ...
        Ok((remnant, Identifier { name: name.to_string(), phantom: PhantomData }))
        // parse_id(input)
    }
}
