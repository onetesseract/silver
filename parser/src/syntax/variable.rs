    ...
    pub fn parse(lexer: Lexer<'a>, state: ParserState) -> ParseResult<Expr> {
        ...
        if lexer.peek_char().render() == "(" {
            ...
            if state.data.read().unwrap().prefix_fns.contains(&v.name.render().to_string()) {
                // is a Prefix Fn
                Ok(Expr::new(ExprVal::Call(CallExpr { target: v, inputs: vec![Expr::parse(lexer, state)?]})))
            }
            ...
            // } 
                ...
                Ok(Expr::new(ExprVal::Call(CallExpr {target: VariableExpr::parse_raw(lexer.clone()), inputs: vec![Expr::new(ExprVal::Variable(v))]})))
            } else {
