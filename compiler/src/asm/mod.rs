...
pub fn compile_asm<'a>(compiler: CompilerInstance<'a>, asm: String, error_at: LexString<'a>) -> CompilationResult<'a> {
    ...
    let r = match opcode {
        ...
        "int_cmp_ult" => Value::from_bool_value(compiler.builder.build_int_compare(inkwell::IntPredicate::ULT, variables[0].into_int_value(), variables[1].into_int_value(), "asm_int_cmp_ult")),
        "int_cmp_ugt" => Value::from_bool_value(compiler.builder.build_int_compare(inkwell::IntPredicate::UGT, variables[0].into_int_value(), variables[1].into_int_value(), "asm_int_cmp_ugt")),
        "int_cmp_ule" => Value::from_bool_value(compiler.builder.build_int_compare(inkwell::IntPredicate::ULE, variables[0].into_int_value(), variables[1].into_int_value(), "asm_int_cmp_ule")),
        "int_cmp_uge" => Value::from_bool_value(compiler.builder.build_int_compare(inkwell::IntPredicate::UGE, variables[0].into_int_value(), variables[1].into_int_value(), "asm_int_cmp_uge")),
        ...
        "raw_store_in" => { compiler.builder.build_store(variables[0].into_ptr_value(), variables[1].get_basic_value()); Value::void_value(compiler.compiler.read().unwrap().context)},
        ...
        "int_and" => Value::from_int_value(compiler.builder.build_and(variables[0].into_int_value(), variables[1].into_int_value(), "asm_int_and")),
        "int_or" => Value::from_int_value(compiler.builder.build_or(variables[0].into_int_value(), variables[1].into_int_value(), "asm_int_or")),
        "int_not" => Value::from_int_value(compiler.builder.build_not(variables[0].into_int_value(), "asm_int_not")),
